...
def grade(ctx: KubernetesContext) -> GradeResult:
    ...
    if not ctx.get_secret("app-configs", "app-secret") or not sec_vol or not sec_mount or sec_mount.get("volumeName") != sec_vol:
        return GradeResult(False, 0, 3, "Secret app-secret is not mounted at /etc/secrets/token in pod config-reader")
    if not ctx.get_configmap("app-configs", "app-config") or not cm_vol or not cm_mount or cm_mount.get("volumeName") != cm_vol:
